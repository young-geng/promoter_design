...
class DEN_loss(nn.Module):
    ...
    def setup(self):
        ...
        self.diversity_loss_intermediate_repr_coef = self.config.diversity_loss_intermediate_repr_coef
        ...
        self.target_cell_exp_weight = self.config.target_cell_exp_weight
...
#         self.motif_loss_intermediate_repr_coef = self.param('motif_loss_intermediate_repr_coef', lambda key, shape: jnp.zeros(shape), (1, ))
    ...
    def __call__(self, \
        ...
        # fitness loss
#         predicted_fitness_seq1_pwm = self.target_cell_exp_weight*predicted_expression_seq1_pwm[:, self.diff_exp_cell_ind] - jnp.mean(predicted_expression_seq1_pwm[:, other_cell_inds], axis=1)
#         predicted_fitness_seq2_pwm = self.target_cell_exp_weight*predicted_expression_seq2_pwm[:, self.diff_exp_cell_ind] - jnp.mean(predicted_expression_seq2_pwm[:, other_cell_inds], axis=1)
        predicted_fitness_seq1_samples = self.target_cell_exp_weight*predicted_expression_seq1_samples[:, :, self.diff_exp_cell_ind] - jnp.mean(predicted_expression_seq1_samples[:, :, other_cell_inds], axis=2)
        predicted_fitness_seq2_samples = self.target_cell_exp_weight*predicted_expression_seq2_samples[:, :, self.diff_exp_cell_ind] - jnp.mean(predicted_expression_seq2_samples[:, :, other_cell_inds], axis=2)
...
#                        jnp.mean(jnp.mean(predicted_fitness_seq1_samples, axis=1)) + jnp.mean(jnp.mean(predicted_fitness_seq2_samples, axis=1))
        fitness_loss = jnp.mean(jnp.mean(predicted_fitness_seq1_samples, axis=1)) + jnp.mean(jnp.mean(predicted_fitness_seq2_samples, axis=1))
