...
#         total_diversity_loss = total_diversity_loss[0]
        ...
        # entropy-based loss function that ensures that bases at every position are different between seeds
        mean_base_usage_at_each_position = jnp.mean(seq1_pwm, axis=0)
        base_entropy_loss_at_each_position = - jnp.sum(-mean_base_usage_at_each_position * jnp.log2(mean_base_usage_at_each_position), axis=1)
        base_entropy_loss_at_each_position = jnp.mean(base_entropy_loss_at_each_position) * self.base_entropy_loss_coef
...
#         entropy_loss = entropy_loss[0]
        ...
        # total loss
        total_loss = fitness_loss + total_diversity_loss + entropy_loss + base_entropy_loss + base_entropy_loss_at_each_position
        
        return total_loss, fitness_loss, total_diversity_loss, entropy_loss, base_entropy_loss, base_entropy_loss_at_each_position, diversity_loss, intermediate_repr_loss