...
def main(argv):
    ...
    print("Optimizing for expression in {} using fitness weight {}".format(id_to_cell[FLAGS.loss_config_updates.diff_exp_cell_ind], FLAGS.loss_config_updates.target_cell_exp_weight))
    ...
    if (not FLAGS.use_existing_checkpoint) or (not os.path.exists(os.path.join(logger.output_dir, 'best_params.pkl'))):
        for step in trange(FLAGS.total_steps, ncols=0):
            ...
            if step % FLAGS.eval_freq == 0:
                ...
                if eval_metrics['eval/fitness_loss'] < best_val_fitness and step >= 1000:
