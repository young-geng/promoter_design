...
class DENValidationDataset:
    def __init__(self, batch_size, latent_size, num_samples=5000):
...
def main(argv):
    ...
    np.save(os.path.join(logger.output_dir, 'all_y.npy'), all_y)
    np.save(os.path.join(logger.output_dir, 'all_yhat.npy'), all_yhat)
    ...
    def train_step(train_state, rng, batch1, batch2):
        ...
        def loss_fn(params, rng, batch1, batch2):
            ...
            entropy_loss, \
            base_entropy_loss, \
            base_entropy_loss_at_each_position, \
            diversity_loss, \
            ...
            aux_values = {
                ...
                "base_entropy_loss_at_each_position": base_entropy_loss_at_each_position, 
        ...
        metrics = jax_utils.collect_metrics(
            aux_values,
            ['fitness_loss', 'total_diversity_loss', 'entropy_loss', 'base_entropy_loss', 'base_entropy_loss_at_each_position', 'diversity_loss', 'intermediate_repr_loss', 'loss',
    ...
    val_dataset = DENValidationDataset(FLAGS.batch_size, generator_latent_size, FLAGS.num_sequences_to_generate)
    ...
    while batch is not None:
        ...
        val_sequences.append(samples)

        batch = next(val_iterator)
    
    val_sequences = np.concatenate(val_sequences, axis=0)
    print("Have {} val sequences".format(val_sequences.shape))
    ...
    np.save(os.path.join(logger.output_dir, 'all_predicted_exps_optim_val_set.npy'), all_predicted_exps)
    np.save(os.path.join(logger.output_dir, 'final_sequences.npy'), val_sequences)
    ...
    final_sequences_predicted_exps = np.stack([
        all_predicted_exps['THP1'],
        all_predicted_exps['Jurkat'],
        all_predicted_exps['K562']
    ], axis=-1)
