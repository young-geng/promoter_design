...
#         self.motif_loss_intermediate_repr_coef = self.param('motif_loss_intermediate_repr_coef', lambda key, shape: jnp.zeros(shape), (1, ))
    ...
    def __call__(self, \
        ...
        if self.diff_exp_cell_ind == 0: # THP1
            fitness_loss = jnp.mean(jnp.mean(fitness_loss_fn(predicted_expression_seq1_samples, self.diff_exp_cell_ind, other_cell_inds, target_cell_weight=1.3), axis=1))
            fitness_loss += jnp.mean(jnp.mean(fitness_loss_fn(predicted_expression_seq2_samples, self.diff_exp_cell_ind, other_cell_inds, target_cell_weight=1.3), axis=1))
        else:
