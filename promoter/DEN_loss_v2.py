...
#         self.motif_loss_intermediate_repr_coef = self.param('motif_loss_intermediate_repr_coef', lambda key, shape: jnp.zeros(shape), (1, ))
    ...
    def __call__(self, \
        ...
        for cell_ind in other_cell_inds:
            predicted_fitness_seq1_pwm += predicted_expression_seq1_pwm[:, self.diff_exp_cell_ind] / nn.relu(predicted_expression_seq1_pwm[:, cell_ind])
            predicted_fitness_seq2_pwm += predicted_expression_seq2_pwm[:, self.diff_exp_cell_ind] / nn.relu(predicted_expression_seq2_pwm[:, cell_ind])
            predicted_fitness_seq1_samples += predicted_expression_seq1_samples[:, :, self.diff_exp_cell_ind] / nn.relu(predicted_expression_seq1_samples[:, :, cell_ind])
            predicted_fitness_seq2_samples += predicted_expression_seq2_samples[:, :, self.diff_exp_cell_ind] / nn.relu(predicted_expression_seq2_samples[:, :, cell_ind])
